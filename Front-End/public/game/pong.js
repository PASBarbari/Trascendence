...
function renderPong() {
  ...
  document
    ...
    .addEventListener("click", showGameModeMenu);
  document
    .getElementById("settingsButton")
    .addEventListener("click", showSettingsMenu);
  document.getElementById("exitButton").addEventListener("click", exitGame);
  document
    .getElementById("onePlayerButton")
    .addEventListener("click", startOnePlayerGame);
  document
  ...
  if (deltaTime >= timeStep) {
    ...
    if (!state.isPaused) {
      if (p1IsHit()) {
        ...
        if (state.ball_speed < 5 * state.player.h) state.ball_speed += 0.1;
      } else if (p2IsHit()) {
...
document.addEventListener("keydown", function (event) {
  if (event.key.toLowerCase() == "w") state.p1_move_y = state.player_speed;
  if (event.key.toLowerCase() == "s") state.p1_move_y = -state.player_speed;
...
document.addEventListener("keyup", function (event) {
  if (event.key.toLowerCase() == "w") state.p1_move_y = 0;
  if (event.key.toLowerCase() == "s") state.p1_move_y = 0;
  if (event.key == "ArrowUp" && !state.IAisActive) state.p2_move_y = 0;
  if (event.key == "ArrowDown" && !state.IAisActive) state.p2_move_y = 0;
});
