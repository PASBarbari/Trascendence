...
async function blockUser(userId, username) {
	...
	try {
		const response = await fetch(`${url_api}/chat/chat/block_user/${userId}`, {
			method: "POST",
			headers: {
		...
		showAlertForXSeconds("Network error while blocking user", "error", 3, {
			asToast: true,
		});
		return false;
	}
...
async function unblockUser(userId, username) {
	...
	try {
		...
		showAlertForXSeconds("Network error while unblocking user", "error", 3, {
			asToast: true,
		});
		return false;
	}
...
async function getBlockedUsersList() {
	...
	try {
		const response = await fetch(`${url_api}/chat/chat/blocked_users/`, {
			method: "GET",
			headers: {
				"Content-Type": "application/json",
				"X-CSRFToken": getCookie("csrftoken"),
...
async function loadBlockedUsers(blockedUserListModal) {
	...
	listContainer.innerHTML = blockedUsers
		.map(
			(user) => `
