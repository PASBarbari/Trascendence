...
class PongWebRTC {
    ...
    setupPeerConnection() {
        ...
        this.peerConnection.onconnectionstatechange = () => {
            ...
            if (state === 'connected') {
                ...
                console.log(`🧊 Final ICE candidate stats: ${this.iceCandidatesGathered} candidates, Types: [${Array.from(this.iceTypes).join(', ')}]`);

                // Show success notification
                this.showConnectionSuccessNotification();
            } else if (state === 'failed' || state === 'disconnected') {
                console.error(`❌ WebRTC connection ${state}`);
                console.log(`🧊 ICE stats at failure: ${this.iceCandidatesGathered} candidates, Types: [${Array.from(this.iceTypes).join(', ')}]`);
                this.handleConnectionFailure();
            } else if (state === 'connecting') {
                console.log(`⏳ WebRTC connection attempt #${this.reconnectAttempts + 1}`);
    ...
    initializeSignaling(room_id) {
        ...
        this.signalingSocket.onclose = (event) => {
            console.log(`📞 Signaling WebSocket chiuso: ${event.code} - ${event.reason}`);
    ...
    waitForInitialICECandidates(timeout = 3000) {
        return new Promise((resolve) => {
            ...
            const checkCandidates = () => {
                ...
                if (this.iceCandidatesGathered >= 2 || elapsed >= timeout ||
                    this.peerConnection.iceGatheringState === 'complete') {
                    console.log(`🧊 ICE candidates ready: ${this.iceCandidatesGathered} candidates after ${elapsed}ms`);
                    resolve();
                } else {
    ...
    handleGameMessage(message) {
        ...
        if (this.stats.messagesReceived % 60 === 0) {
            console.log(`📡 WebRTC messaggi: ${this.stats.messagesReceived} ricevuti`);
        }
    ...
    applyPaddlePosition(player, position) {
        ...
        if (state.players[paddleIndex]) {
            ...
            state.players[paddleIndex].mesh.position.set(position[0], position[1], position[2]);
        }
    }
    ...
    applyBallState(position, velocity) {
        ...
        const isValidPosition = Math.abs(position[0]) < maxX && Math.abs(position[2]) < maxY;

        if (!isValidPosition) {
            console.warn("🚨 Invalid ball position received, ignoring:",
    ...
    applyScoreUpdate(p1_score, p2_score) {
        ...
        import("./locale/src/Score.js").then(({ updateScore }) => {
            if (updateScore) {
                ...
                updateScore("p1");
                updateScore("p2");
                console.log(`✅ Score UI updated successfully: P1=${p1_score}, P2=${p2_score}`);
            } else {
    ...
    handlePlayerReady(data) {
        ...
        if (readyStatus) {
            readyStatus.innerHTML = '<small><i class="fas fa-check text-success me-2"></i>Other player is ready!</small>';
        }
    ...
    startGame() {
        ...
        setTimeout(() => {
            if (window.updateRoleIndicator && state.isHost !== undefined) {
                ...
                console.log('🔄 Role indicator updated on connection:', state.isHost ? 'Host' : 'Guest');
            }
        }, 100);
...
export function webrtcGameLoop() {
    ...
    if (state.isHost && state.ball) {
        ...
        if (!connection.hostBoundariesLogged) {
            ...
            console.log("🏟️ HOST Ring dimensions:", { length: state.ring.length, height: state.ring.height });
        ...
        if (isValidPosition) {
            ...
            console.warn("🚨 HOST: Invalid ball position, not sending:",
                `(${pos.x.toFixed(1)}, ${pos.z.toFixed(1)})`,
                `max: (±${maxX.toFixed(1)}, ±${maxY.toFixed(1)})`);
            console.warn("🚨 HOST: Ball velocity:", `(${vel.x.toFixed(1)}, ${vel.z.toFixed(1)})`);
        }
    }
}
    ...
    if (isInitiator) {
        const waitForSignaling = (attempts = 0) => {
            ...
            if (connection.signalingSocket && connection.signalingSocket.readyState === WebSocket.OPEN) {
                console.log('📞 Signaling ready, waiting for ICE gathering before starting connection...');
                ...
                console.log(`⏳ Waiting for signaling socket to open... (attempt ${attempts + 1})`);
                if (connection.signalingSocket) {
                    connection.signalingSocket.addEventListener('open', () => {
                        console.log('📞 Signaling opened, waiting before starting connection...');
                        setTimeout(() => {
                            console.log('🚀 Starting WebRTC offer creation...');
                            connection.startConnection();
