...
export function moveIA() {
    ...
    while (timeElapsed < delay) {
        simulatedBallY += state.ball_speed * Math.sin(simulatedAngle * Math.PI / 180) * timeStep;
        simulatedBallX += state.ball_speed * Math.cos(simulatedAngle * Math.PI / 180) * timeStep;
        timeElapsed += timeStep;

        if (simulatedBallY + state.ball_radius + state.ball_speed > state.ring.x / 2 || simulatedBallY - state.ball_radius + state.ball_speed < -state.ring.x / 2) {
            simulatedAngle *= -1;
        }
    }
