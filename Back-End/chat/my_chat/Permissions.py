...
class IsChatMember(BasePermission):
    ...
    def has_permission(self, request, view):
        ...
        chat_id = view.kwargs.get('chat_id') or request.data.get('chat_id') or request.query_params.get('chat_id')
        if not chat_id:
            return False
            
        # Check cache first
        cache_key = f"chat_membership_{request.user.id}_{chat_id}"
    ...
    def has_object_permission(self, request, view, obj):
        ...
        if hasattr(obj, 'chat_room_id'):
            chat_id = obj.chat_room_id
        elif hasattr(obj, 'chat_id'):
            chat_id = obj.chat_id
        else:
            # If it's a chat room itself
            chat_id = getattr(obj, 'id', None)
...
class IsChatAdmin(BasePermission):
    ...
    def has_permission(self, request, view):
        ...
        # For actions that should allow all admins (even with no chat_id)
        if getattr(view, 'action', None) in ['list', 'create']:
            return True
        ...
        # Check cache first
        cache_key = f"chat_admin_{request.user.id}_{chat_id}"
...
class CanSendMessage(BasePermission):
    ...
    def has_permission(self, request, view):
        ...
        chat_id = view.kwargs.get('chat_id') or request.data.get('chat_id')
        if not chat_id:
            return False
            
        # Check cache first
        cache_key = f"can_send_message_{request.user.id}_{chat_id}"
...
class ReadOnlyPermission(BasePermission):
    """
    Permission that allows read-only access to chat resources.
    """
    def has_permission(self, request, view):
